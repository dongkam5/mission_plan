...
with col_left:
    ...
    with tab_intel:
        st.subheader("위협 추가")
        # st.markdown("**위협 추가**")  # 일반 텍스트 크기, 굵게
        add_type = st.radio('유형', ["원형 (SAM)", "사각형 (NFZ)"], horizontal=True,  label_visibility="collapsed" )  # 라벨과 라벨 자리 모두 제거
        ...
        # 위협 목록
        st.subheader("위협 목록")  # 일반 텍스트 크기, 굵게
        if mission.threats:
            threat_df = pd.DataFrame([t.to_dict() for t in mission.threats])
            st.dataframe(threat_df, hide_index=True, width='content')
...
with col_right:
    ...
    raw_in = []
    if wp_coord:
        p1 = pathfinder.find_path(start_coord, wp_coord, threats_dict, mission.params.margin) if hasattr(pathfinder, 'find_path') else pathfinder.find_path_3d(start_coord, wp_coord, threats_dict, mission.params.margin)
        p2 = pathfinder.find_path(wp_coord, target_coord, threats_dict, mission.params.margin) if hasattr(pathfinder, 'find_path') else pathfinder.find_path_3d(wp_coord, target_coord, threats_dict, mission.params.margin)
        if p1 and p2:
            raw_in = p1 + p2[1:]
    else:
    ...
    if mission.params.rtb and final_in:
        if hasattr(pathfinder, 'find_path_3d') and mission.params.enable_3d:
            raw_out = pathfinder.find_path_3d(target_coord, start_coord, threats_dict, mission.params.margin)
            if raw_out:
    ...
    if final_in:
        ...
        st.dataframe(stpt_df, width='content', hide_index=True)
